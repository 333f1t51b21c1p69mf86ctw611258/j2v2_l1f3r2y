...
<fieldset>
	...
	<c:if
		test='<%=SessionMessages.contains(renderRequest.getPortletSession(), "feedback-submit-success")%>'>
		<liferay-ui:success key="feedback-submit-success"
			message="Claim has been submited successfully." />
	</c:if>
	<c:if
		test='<%=SessionMessages.contains(renderRequest.getPortletSession(), "workflow-not-enabled")%>'>
		<liferay-ui:success key="workflow-not-enabled"
			message="Claim not enabled with workflow please enable workflow." />
	</c:if>
	<c:if
		test='<%=SessionErrors.contains(renderRequest.getPortletSession(), "feedback-submit-failed")%>'>
	...
	<aui:form action="<%=submitFeedBack%>" method="post"
		...
		<aui:layout>
			<aui:column>
				<aui:input label="Claim Subject" name="feedbackSubject"
		...
		<aui:layout>
			<aui:input name="feedbackMessage" id="feedbackMessage"
				label="Claim Message" type="textarea"
		...
		<aui:layout>
			<aui:column>
				<aui:button type="submit" value="Submit Claim" name="submit"></aui:button>
			</aui:column>
		</aui:layout>
	</aui:form>
</fieldset>
