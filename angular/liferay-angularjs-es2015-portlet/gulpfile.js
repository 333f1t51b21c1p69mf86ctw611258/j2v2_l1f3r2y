...
gulp.task('js-check', function() {
    return gulp.src('src/main/webapp/client/javascript/**/*.js')
        .pipe(eslint())
        .pipe(eslint.format())
...
gulp.task('js-build', [ 'js-check' ], function() {
    browserify('src/main/webapp/client/javascript/boot.js', { debug: true })
...
gulp.task('js-watch', [ 'js-check' ], function() {
    const bundler = watchify(browserify('src/main/webapp/client/javascript/boot.js', { debug: true })
        .transform(babelify, { presets: [ "es2015"] }))
        .on('update', function() {
...
gulp.task('sass-compile', function() {
    return gulp.src('src/main/webapp/client/sass/**/*.scss')
...
gulp.task('sass-watch', [ 'sass-compile'], function() {
    gulp.watch('src/main/webapp/client/sass/**/*.scss', [ 'sass-compile' ]);
});
